...
const ProductActions = ({ productId, stock }) => {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        // Fetch Wishlist
        const wishlistRes = await axios.get('meeyalbackendnode-production.up.railway.app/api/wishlist', {
        ...
        // Fetch Cart
        const cartRes = await axios.get('meeyalbackendnode-production.up.railway.app/api/cart', {
          withCredentials: true,
        });
  ...
  const handleAction = async (action) => {
    try {
      if (action === 'Add to Cart') {
        if (inCart) {
          // Remove from cart
          const res = await axios.delete('meeyalbackendnode-production.up.railway.app/api/cart/remove', {
            data: { productId },
            withCredentials: true,
          });
          ...
          const res = await axios.post(
            'meeyalbackendnode-production.up.railway.app/api/cart/add',
      ...
      if (action === 'Add to Wishlist') {
        if (inWishlist) {
          const res = await axios.delete('meeyalbackendnode-production.up.railway.app/api/wishlist', {
            data: { productId },
            withCredentials: true,
          });
          ...
          const res = await axios.post(
            'meeyalbackendnode-production.up.railway.app/api/wishlist',
            { productId },
            { withCredentials: true }
          );
